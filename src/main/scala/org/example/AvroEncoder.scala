...
import org.example.SchemaConverters.{SchemaType, resolveUnionType, toSqlType}
    ...
    avroSchema.getType match {
      ...
      case UNION =>
        val (resolvedSchema, _) =
          SchemaConverters.resolveUnionType(avroSchema, Set.empty)
  ...
  private case class ObjectCast(
    ...
    override def mapChildren(f: Expression => Expression): Expression = {
      if (containsChild.nonEmpty) {
        legacyWithNewChildren(children.map(f))
      } else {
        this
      }
    }
